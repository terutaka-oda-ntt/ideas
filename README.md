...
| [開発環境セットアップ](guardrails/docs/dev-environment.md) | pre-commit・detect-secrets のローカル運用 |
...
- `guardrails/scripts/apply_org_branch_protection.sh` の適用プロファイル
...
- **detect-secrets**: ローカル pre-commit で APIキー・DB接続情報・認証情報を検出
- **gitleaks-pr-scan**: CI で PR 差分のシークレット検査を実行
...
ブランチ保護の標準適用は
[`guardrails/scripts/apply_org_branch_protection.sh`](guardrails/scripts/apply_org_branch_protection.sh) を利用します。

低レベルなパラメータを直接指定する場合は
...
既定では `main` に `gitleaks-pr-scan` を必須チェックとして設定します。
`full-guardrails` プロファイルを使うと `quality-and-security` も必須にできます。
...
└── guardrails/
    ...
    │   └── project-start.md
    ├── scripts/
    │   ├── apply_branch_protection.sh     # ブランチ保護の低レベル適用
    │   ├── apply_org_branch_protection.sh # 組織標準プロファイル適用
