...
このドキュメントの前半は GitHub ベースの共通ガードレールとして利用できます。後半の Azure 部分は参考実装です。
...
- 対象イベント:
    - `main` への `pull_request`
    - `main` への `push`
- 実行内容:
    - `gitleaks-pr-scan`: PR差分に対するシークレット検査
    - `quality-and-security`: `pre-commit run --all-files --show-diff-on-failure` と `./guardrails/test/run_tests.sh`
...
GitHubのブランチ保護設定で `main` に以下を適用します。

- 必須ステータスチェックとして `gitleaks-pr-scan` を設定
...
`quality-and-security` も必須にする場合は、full-guardrails プロファイルを利用します。
...
対象ブランチを拡張する場合は `--branches` を指定します。

```bash
export GH_TOKEN=<repo admin token>
./guardrails/scripts/apply_org_branch_protection.sh --branches main,release
```

### 権限チェックリスト（main ブランチ保護）
...
- `Required approving reviews` はリポジトリ方針に従う（0以上）
- `Require status checks to pass before merging` が有効
- 必須チェックに `gitleaks-pr-scan` が含まれる
...
- `main` 向けPRで `gitleaks-pr-scan` が必須チェックとして表示される
...
# main の保護有効状態と必須チェック（gitleaks-pr-scan）を確認
for b in main; do
    ...
    has_gl=$(echo "$json" | jq -r '(.protection.required_status_checks.contexts // []) | index("gitleaks-pr-scan") != null')
    echo "$b protected=$protected status_check_gitleaks-pr-scan=$has_gl"
done
```
...
- **手動デプロイの場合**: releaseブランチや環境ブランチへのプルリクエストとしてチケットを作成
...
このセクション以降はテンプレート本体の必須要素ではなく、Azure 利用時の参考情報です。
